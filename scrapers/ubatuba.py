...
def main(search_term: str):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("logs/scraping_ubatuba.log", encoding="utf-8"),
            logging.StreamHandler(),
        ],
    )

    # SEARCH_URL = "https://www.ubatuba.sp.gov.br/page/{}/?s=Educa%C3%A7%C3%A3o+Ambiental"
    SEARCH_URL = "https://www.ubatuba.sp.gov.br/page/{}/?s=" + quote_plus(search_term)
    ...
    pagination = sel.xpath(".//ul[contains(@class,'pagenavi')]/li")
    if pagination:
    ...
    noticias = sel.xpath(
        ".//ul[contains(@class, 'blog-items')]/.//div[contains(@class,'search-item-content')]"
    )
    ...
    for noticia in noticias:
        titulo = clean_text(noticia.xpath(".//h3/a/text()").get())
        logging.info(f"Processando notícia: {titulo}")
        logging.debug(f"Título encontrado: {titulo}")

        link = clean_text(noticia.xpath(".//h3/a/@href").get())
        logging.debug(f"Link encontrado: {link}")

        data = clean_text(noticia.xpath(".//time/text()").get())
        ...
        resumo = clean_text(
            noticia.xpath(".//div[contains(@class, 'excerpt')]/text()").get()
        )
    ...
    with open("json/dados_ubatuba_bruto.json", "w", encoding="utf-8") as f:
        json.dump(resultados, f, ensure_ascii=False, indent=2)
    logging.info("Arquivo teste.json salvo com sucesso.")
    pd.DataFrame(resultados).to_excel("xlsx/dados_ubatuba_bruto.xlsx")
    return "json/dados_ubatuba_bruto.json"
