...
class UbatubaScraper(ScraperStrategy):
    ...
    PAGINATION_XPATH = ".//ul[contains(@class,'pagenavi')]/li"

    NEWS_XPATH = (
        ".//ul[contains(@class, 'blog-items')]"
        "/.//div[contains(@class,'search-item-content')]"
    )
    ...
    DATA_XPATH = ".//time/text()"

    RESUMO_XPATH = ".//div[contains(@class, 'excerpt')]/text()"
    ...
    def __extract_data(
        ...
        data = self.__clean_text(noticia.xpath(self.DATA_XPATH).get())
